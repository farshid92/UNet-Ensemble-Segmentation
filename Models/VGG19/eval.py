...
def read_image(path):
    ...
    x = cv2.resize(x, (W, H))
    ori_x = x.copy()  # Save original image for later
    x = x / 255.0
...
def read_mask(path):
    ...
    ori_x = x.copy()  # Save original mask for later
    x = x / 255.0
...
def save_results(ori_x, ori_y, y_pred, save_image_path):
    ...
    # Convert to 3-channel images for visualization
...
if __name__ == "__main__":
    ...
    results_dir = "results_model_vgg19_unet"
    ...
    if SCORE:
        ...
        df.to_csv("files/score_vgg19_unet.csv")
