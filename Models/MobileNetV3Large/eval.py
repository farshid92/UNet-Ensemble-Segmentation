...
def save_results(ori_x, ori_y, y_pred, save_image_path):
      ...
      # Check dimensions
    # print(f"ori_x shape: {ori_x.shape}")
    # print(f"ori_y shape: {ori_y.shape}")
...
if __name__ == "__main__":
    ...
    results_dir= "results_mobilenetv3_large"
    create_dir(results_dir)
    
    """load the model"""
    with CustomObjectScope({'iou': iou, 'dice_coef': dice_coef, 'dice_loss': dice_loss}):
        model = tf.keras.models.load_model("files/model_mobilenetv3large.keras")
    ...
    if SCORE:
        ...
        df.to_csv("files/score_mobilenetv3_large.csv")
